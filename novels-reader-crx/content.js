...
var _pageAnalyzer = __webpack_require__(171);
...
var lineUnHighlight = function lineUnHighlight() {
  analyzer.highlightElements.removeClass('highlight');
};

if (analyzer.readElemtns.body.length) {
  chrome.runtime.sendMessage({ method: 'getOptions', key: 'options' }, function (responseOptions) {
    chrome.runtime.sendMessage({ method: 'saveDictionary', dictionary: {
      ...
      for (var key in analyzer.readElemtns) {
        if (options[key] == 'on' && analyzer.readElemtns[key].length) {
          var filteredElements = analyzer.readElemtns[key].filter(function (index, element) {
...
var _assign2 = __webpack_require__(156);
...
var _narou = __webpack_require__(172);
...
var PageAnalyzer = function () {
  ...
  _createClass(PageAnalyzer, [{
    ...
    value: function getDictionaryText() {
      var dictionaryText = '';
      this.rubies.forEach(function (ruby) {
    ...
    key: 'checkIncludeRuby',
    value: function checkIncludeRuby(text) {
      return (/<ruby><rb>/gi.test(text)
      );
    }
  }, {
    key: 'checkIgnoreRubiesTest',
    value: function checkIgnoreRubiesTest(ruby, dictionaries) {
      return dictionaries.ignoreRubies && dictionaries.ignoreRubies.raw && RegExp(dictionaries.ignoreRubies.raw, 'gi').test(ruby.rt);
    }
  }, {
    key: 'setRubyData',
    value: function setRubyData($lineElement, dictionaries) {
      var _this = this;
      ...
      if (this.checkIncludeRuby($lineElement.html())) {
        ...
        var readText = splitRubyTagTexts.map(function (splitRubyTagText) {
          if (_this.checkIncludeRuby(splitRubyTagText)) {
            var ruby = { rb: $(splitRubyTagText).find('rb').text(), rt: $(splitRubyTagText).find('rt').text() };
            if (!(0, _find3.default)(_this.rubies, ruby) && !_this.checkIgnoreRubiesTest(ruby, dictionaries)) {
              _this.rubies.push(ruby);
            }
            return _this.checkIgnoreRubiesTest(ruby, dictionaries) ? ruby.rb : ruby.rt;
          } else {
    ...
    key: 'setPlayButtonElementsAndSetRubyData',
    value: function setPlayButtonElementsAndSetRubyData(lineElements, dictionaries) {
      var _this2 = this;

      lineElements.each(function (index, lineElement) {
        var $lineElement = $(lineElement);
        _this2.setRubyData($lineElement, dictionaries);
        $lineElement.prepend($('<div class=\'controll-button play\' data-index=\'' + _this2.lineIndex++ + '\'><i class=\'fa fa-play-circle\' aria-hidden=\'true\'></div>'));
      });
    }
  }]);
